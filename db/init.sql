...
CREATE TABLE IF NOT EXISTS job_feedbacks (
  ...
  level NUMERIC CHECK (level > 0 AND level <= 5),
  end_time TIME,
  holiday NUMERIC CHECK (holiday >= 0),
  overtime_hours NUMERIC CHECK (overtime_hours >= 0),
  age NUMERIC CHECK (age >= 0),
  tenure_years NUMERIC CHECK (tenure_years >= 0),
  marriage_age NUMERIC CHECK (marriage_age >= 0),
