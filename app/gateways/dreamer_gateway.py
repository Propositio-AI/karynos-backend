...
class DreamerGateway(BasePrismaGateway):
    ...
    def create_dreamer(
        self, payload: prisma_types.DreamerCreateInput | dict[str, Any]
    ) -> GatewayResult[Dreamer]:
    ...
    def find_by_identity(
        self, name_family: str, name_given: str, grade: str | None
    ...
    def update_dreamer(
        ...
        return self.update_many_and_fetch(
            self.DREAMER, {"dreamer_id": str(dreamer_id)}, payload
        )

    def delete_dreamer(self, dreamer_id: str) -> GatewayResult[list[Dreamer]]:
        return self.delete_many_and_return_before(
            self.DREAMER, {"dreamer_id": str(dreamer_id)}
        )

    def create_group(
        self, payload: prisma_types.DreamerGroupCreateInput | dict[str, Any]
    ) -> GatewayResult[DreamerGroup]:
    ...
    def update_group(
        ...
        return self.update_many_and_fetch(
            self.GROUP, {"group_id": str(group_id)}, payload
        )

    def delete_group(self, group_id: str) -> GatewayResult[list[DreamerGroup]]:
        return self.delete_many_and_return_before(
            self.GROUP, {"group_id": str(group_id)}
        )

    def list_members_by_dreamer(
        self, dreamer_id: str
    ) -> GatewayResult[list[DreamerGroupMember]]:
    ...
    def delete_members_by_dreamer(
        self, dreamer_id: str
    ) -> GatewayResult[list[DreamerGroupMember]]:
        return self.delete_many_and_return_before(
            self.MEMBER, {"dreamer_id": str(dreamer_id)}
        )

    def delete_members_by_group(
        self, group_id: str
    ) -> GatewayResult[list[DreamerGroupMember]]:
        return self.delete_many_and_return_before(
            self.MEMBER, {"group_id": str(group_id)}
        )

    def delete_group_member(
        self, group_id: str, dreamer_id: str
    ) -> GatewayResult[list[DreamerGroupMember]]:
        return self.delete_many_and_return_before(
            self.MEMBER,
    ...
    def list_init_questions(self, version: int) -> GatewayResult[list[InitQuestion]]:
        return self.find_many(
            self.INIT_QUESTION, {"version": int(version), "is_active": True}
        )

    def list_question_options(
        self, question_id: str
    ) -> GatewayResult[list[InitQuestionOption]]:
        return self.find_many(self.INIT_OPTION, {"question_id": str(question_id)})

    def list_answer_history(
        self, dreamer_id: str
    ) -> GatewayResult[list[UserInitialAnswer]]:
    ...
    def get_answer_question(
        self, question_id: str
    ) -> GatewayResult[list[InitQuestion]]:
