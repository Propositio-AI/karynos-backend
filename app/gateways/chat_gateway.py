...
class ChatGateway(BasePrismaGateway):
    ...
    def list_conversations_by_owner(
        ...
        return self.find_many(
            self.CONVERSATION, {"owner_id": str(owner_id)}, order={"created_at": "asc"}
        )
    ...
    def get_conversation(
        ...
        return self.find_many(
            self.CONVERSATION, {"conversation_id": str(conversation_id)}
        )
    ...
    def update_conversation(
        ...
        return self.update_many_and_fetch(
            self.CONVERSATION, {"conversation_id": str(conversation_id)}, update_data
        )
    ...
    def delete_conversation_participants(
        self, conversation_id: str
    ) -> GatewayResult[list[ConversationParticipant]]:
        return self.delete_many_and_return_before(
            self.PARTICIPANT, {"conversation_id": str(conversation_id)}
        )

    def create_message(
        self, payload: prisma_types.MessageCreateInput | dict[str, Any]
    ...
    def delete_messages_by_conversation(
        ...
        return self.delete_many_and_return_before(
            self.MESSAGE, {"conversation_id": str(conversation_id)}
        )

    def delete_message(self, message_id: str) -> GatewayResult[list[Message]]:
        return self.delete_many_and_return_before(
            self.MESSAGE, {"message_id": str(message_id)}
        )
