...
from app.services.chat.job_api import get_job_data
...
def get_messages_for_openai(conversation_id: str, crud):
    ...
    if not response["success"]:
        raise Exception(
            f"会話履歴の取得に失敗しました: {', '.join(response['message'])}"
        )
...
def create_system_prompt(job_data: dict, prompt_path: str | None = None) -> str:
    if prompt_path is None:
        prompt_path = str(
            Path(__file__).resolve().parent / "prompts" / "character_prompt.txt"
        )
    context = {
        "ai_name": "アシスタント",
        "ai_gender": "unisex",
        ...
        "skills_str": ", ".join(
            [
                s.get("name", "")
                for s in job_data.get("skills", [])
                if s.get("is_required", True)
            ]
        ),
        "certifications_str": ", ".join(
            [c.get("name", "") for c in job_data.get("certifications", [])]
        ),
        "interests_str": ", ".join(
            [i.get("name", "") for i in job_data.get("interests", [])]
        ),
        "talents_str": ", ".join(
            [t.get("name", "") for t in job_data.get("talents", [])]
        ),
        "companies_str": ", ".join(
            [c.get("name", "") for c in job_data.get("companies", [])]
        ),
    }
