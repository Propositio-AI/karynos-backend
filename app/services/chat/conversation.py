...
from app.services.chat.character import generate_name
from app.services.chat.context import build_openai_messages
from app.services.chat.job_api import get_job_data, normalize_job_id
...
class ConversationService:
    async def create_conversation(
        self, request: CreateConversationRequest, user_id: UUID
    ):
    ...
    def get_conversation_details(self, conversation_id: str):
        ...
        if not conversation_response["success"] or not conversation_response["data"]:
            raise HTTPException(
                status_code=404,
                detail=self._detail_from_message(
                    conversation_response, "Conversation not found"
                ),
            )
    ...
    def delete_conversation(self, conversation_id: str):
        participants_response = chat_gateway.delete_conversation_participants(
            conversation_id
        )
        self._ensure_success(participants_response)

        messages_response = chat_gateway.delete_messages_by_conversation(
            conversation_id
        )
        self._ensure_success(messages_response)
    ...
    async def create_ai_response_stream(
        ...
        if not create_message_response["success"]:
            detail = self._detail_from_message(
                create_message_response, "Failed to create message"
            )
            yield f"event: error\ndata: {detail}\n\n"
            return
        ...
        if not update_response["success"]:
            detail = self._detail_from_message(
                update_response, "Failed to update conversation"
            )
            yield f"event: error\ndata: {detail}\n\n"
            return
        ...
        try:
            ...
            if ai_message_content:
                ...
                if not ai_response["success"]:
                    print(
                        self._detail_from_message(
                            ai_response, "Failed to save ai response"
                        ),
                        flush=True,
                    )
